...
class OrderCreate(CreateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["options_used"].queryset = TripOptionAvailable.objects.filter(
            trip=form.initial["trip"]
        )
        return form
        ...
        if form_obj.trip.subbed > form_obj.trip.max_group_size:
            form.add_error(
                ...
                forms.ValidationError(
                    "Количество людей в заявке превышает количество оставшихся мест!"
                ),
            )
            form.add_error(
                "kids_amount",
