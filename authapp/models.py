...
class AppUser(AbstractUser):
    ...
    gender = models.IntegerField(
        verbose_name="Пол", blank=False, db_index=True, choices=Gender.choices
    )
    phone = models.CharField(verbose_name="Номер телефона", blank=True, max_length=20)
    email = models.EmailField(
        verbose_name="Адрес Email", unique=True, validators=[email_is_lowercase]
    )
    ...
    def __str__(self):
        return (
            f"[{self.get_gender_display()[:1]}] {self.username} ({self.role_display})"
        )
...
class Traveler(models.Model):
    ...
    user = models.OneToOneField(
        AppUser, unique=True, db_index=True, null=False, on_delete=models.CASCADE
    )
    ...
    following = models.ManyToManyField(
        "Instructor", verbose_name="Подписки", blank=True, related_name="followers"
    )
    trips_completed = models.ManyToManyField(
        "travelapp.Trip", verbose_name="Походы", related_name="participants"
    )
...
class Instructor(models.Model):
    ...
    user = models.OneToOneField(
        AppUser, unique=True, db_index=True, null=True, on_delete=models.CASCADE
    )
    about = models.TextField(verbose_name="О себе", blank=True, default="Обо мне")
    home_region = models.CharField(
        verbose_name="Место проживания", max_length=100, blank=True, null=True
